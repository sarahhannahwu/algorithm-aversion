...
coded_table_classes %>% 
  mutate(Algorithm_orientations = case_when(Algorithm_orientations == "Both" ~ "Aversion and appreciation",
  ...
  mutate(Algorithm_orientations = fct_relevel(Algorithm_orientations, c("Aversion", "Appreciation", "Aversion and appreciation", "Neutral"))) %>% 
...
relabeled_domains %>% 
  ...
  labs(x="Task Domain",
       ...
       caption = "Dashed line at y = 0.5 indicates the mean rate of algorithm aversion across all articles in the sample") 
...
broad_tech_categories %>% 
  ...
  group_by(Year) %>% 
  count(broad_tech_category) %>% 
  ggplot(aes(x=Year, y=n, color=broad_tech_category)) +
  geom_line(aes(group=broad_tech_category), alpha=0.5) +
  geom_point(size=2) +
  labs(x="Year",
       y="Number of articles",
       color="Technology") +
  scale_x_continuous(breaks = seq(2015, 2024, by=1)) +
  scale_color_locuszoom(breaks=c("AI", "rule-based algorithm", "machine learning algorithm", "robot", "self-driving vehicle", "chatbot")) +
  theme_light()
```
